...
def completar_registro(request):
    ...
    terminos = TerminosCondiciones.get_vigente(rol='paciente')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Convertir asignaciones pendientes en asignaciones reales
            from cuestionarios.models import AsignacionPendiente, AsignacionCuestionario
            pendientes = AsignacionPendiente.objects.filter(invitacion=invitacion)
            for ap in pendientes:
                AsignacionCuestionario.objects.get_or_create(
                    especialista=ap.especialista,
                    paciente=paciente,
                    cuestionario=ap.cuestionario,
